...
coolant_vals = deque(maxlen=MAX_POINTS)
...
def physics_loop():
    ...
    while True:
        ...
        with lock:
            ...
            coolant_vals.append(round(state["coolant"], 1))
...
scenarios = {
    ...
    "full": [
        ...
        {"dur": 5,  "spd": 130, "bT": 30,     "label": "Full Run [2/3] — Accelerating to highway"},
...
def run_scenario(name):
    global scenario_timer
    steps = scenarios[name]
    def run_step(idx):
        global scenario_timer
        if idx >= len(steps):
            with lock:
                state["scenario"] = "Simulation complete"
            return
        s = steps[idx]
        with lock:
        ...
        scenario_timer = threading.Timer(s["dur"], run_step, args=[idx + 1])
...
# Professional button style builder
def scenario_btn(label, btn_id, accent, bg, n=0):
    return html.Button(label, id=btn_id, n_clicks=n, style={
        "flex": "1",
        "padding": "10px 8px",
        "borderRadius": "8px",
...
def sensor_dot(dot_id, label, default_color="#1D9E75", default_glow="0 0 4px #1D9E75"):
    return html.Div([
        html.Div(id=dot_id, style={
            "width":"8px","height":"8px","borderRadius":"50%",
            "background": default_color,
            "boxShadow": default_glow,
            "transition":"all .4s","flexShrink":"0"
        }),
        html.Span(label, style={
            "fontSize":"10px","fontWeight":"600","color":"#555","letterSpacing":".03em"
        }),
    ], style={
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Span(id="state-badge", children="NORMAL", style={
            ...
            "border":"1.5px solid #1D9E75","transition":"all .5s",
            "letterSpacing":".05em"
        }),
    ], style={
    ...
    html.Div([
        ...
        html.Div([
            ...
            # Scenario buttons — professional, no emojis
            html.Div([
                html.Div("Simulation Scenarios", style={
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Graph(id="graph-fan", config={"displayModeBar":False},
                          style={"height":"130px"}),
            ], style={"background":"#fff","border":"1px solid #e8e8e6",
                      "borderRadius":"12px","padding":"12px 14px","marginBottom":"9px"}),
            ...
            html.Div([
                ...
                dcc.Graph(id="graph-soc", config={"displayModeBar":False},
                          style={"height":"130px"}),
            ], style={"background":"#fff","border":"1px solid #e8e8e6",
                      "borderRadius":"12px","padding":"12px 14px","marginBottom":"9px"}),
...
def run_cold(n):
    threading.Thread(target=run_scenario, args=("cold",),    daemon=True).start(); return n

@app.callback(Output("s-heat","data"),  Input("btn-heat","n_clicks"),  prevent_initial_call=True)
def run_heat(n):
    threading.Thread(target=run_scenario, args=("highway",), daemon=True).start(); return n

@app.callback(Output("s-fault","data"), Input("btn-fault","n_clicks"), prevent_initial_call=True)
def run_fault(n):
    threading.Thread(target=run_scenario, args=("fault",),   daemon=True).start(); return n

@app.callback(Output("s-full","data"),  Input("btn-full","n_clicks"),  prevent_initial_call=True)
def run_full(n):
    threading.Thread(target=run_scenario, args=("full",),    daemon=True).start(); return n
...
def update(n):
    ...
    badge_style = {
        ...
        "border":f"1.5px solid {sc['border']}","transition":"all .5s",
        "letterSpacing":".05em"
    }
    ...
    if d["dtc"]:
        dtc_el = html.Div([
            html.Span("Diagnostic Alert", style={
                "fontSize":"9px","fontWeight":"700","textTransform":"uppercase",
                "letterSpacing":".06em","color":"#993C1D","display":"block","marginBottom":"2px"
            }),
            html.Span(d["dtc"], style={"fontWeight":"600","fontSize":"12px"}),
        ], style={
            "background":"#FDF0ED","border":"1.5px solid #D85A30",
    ...
    def base_layout(h):
        return dict(
            ...
            yaxis=dict(showgrid=True, gridcolor="#f5f5f3", color="#aaa",
    ...
    temp_fig.add_hline(y=45, line=dict(color="rgba(239,159,39,0.5)", width=1, dash="dash"),
                       annotation_text="Cool 45°C", annotation_font_size=9,
                       annotation_font_color="#854F0B")
    temp_fig.add_hline(y=10, line=dict(color="rgba(55,138,221,0.5)", width=1, dash="dash"),
                       annotation_text="Heat 10°C", annotation_font_size=9,
    ...
    soc_fig.add_hline(y=15, line=dict(color="rgba(239,159,39,0.4)", width=1, dash="dash"),
                      annotation_text="Low SoC", annotation_font_size=9,
                      annotation_font_color="#854F0B")
    ...
    if fl:
        fault_els = [
            html.Div([
                html.Span(f"t={f['t']}", style={
                    "fontSize":"10px","color":"#aaa","minWidth":"48px",
        ...
        fault_els = [html.Div("No faults recorded.",
            style={"fontSize":"11px","color":"#bbb","padding":"4px 0",
                   "fontStyle":"italic"})]
